...
Centralises patterns that were previously duplicated between vault/routes.py
and todo/routes.py (Fix #9: eliminate _NO_CACHE string duplication).
"""
...
def no_cache_page(template: str, **ctx) -> ResponseReturnValue:
    ...
    Use for any page that shows user-specific or session-sensitive content
    (vault, todo) so that the browser never serves a stale cached copy
    after logout.
    """
